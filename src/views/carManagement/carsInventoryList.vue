        ...
        <el-row>
          ...
          <el-col :span="8">
            <carBrandSerise @seriseChangeVal="seriseFilterChangeVal" @brandChangeVal="brandFilterChangeVal" @modelChangeVal="modelFilterChangeVal" :oldBrandId="filterBrandId" :oldSeriseId="filterSeriesId"
              :oldModelId="filterModelId" :hasBrandMult="true" :hasSeriesMult="true" :hasModelMult="true" :hasModel="true"></carBrandSerise>
          </el-col>
        </el-row>
        ...
        <el-table :data="tableData" border v-loading="loading">
          ...
          <el-table-column label="操作" width="380" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="primary" plain v-if="buttonLimits.includes(135)" icon="el-icon-document" style="margin-top: 5px" @click="gotoDetail(scope.$index, scope.row)">详情</el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="scope.row.carStatus == 2 && buttonLimits.includes(127)" @click="showSellPriceDialog(scope.$index, scope.row)">
                销售定价</el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="scope.row.carStatus == 2 && buttonLimits.includes(128)" @click="upShelf(scope.$index, scope.row)">上架</el-button>
              <el-button size="mini" type="danger" icon="el-icon-sell" style="margin-top: 5px" v-if="scope.row.carStatus == 3 && buttonLimits.includes(129)" @click="downShelf(scope.$index, scope.row)">下架</el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="
                  scope.row.carStatus == 2 &&
                  (!scope.row.workStatus ||
                    scope.row.workStatus == 7 ||
                    scope.row.workStatus == 8) &&
                  buttonLimits.includes(130)
                " @click="applyHostling(scope.$index, scope.row)">发起整备</el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="
                  scope.row.carStatus == 2 &&
                  scope.row.workStatus == 7 &&
                  !scope.row.certifiedStatus &&
                  buttonLimits.includes(131) && certiAuth == 1
                " @click="applyAuthentication(scope.$index, scope.row)">申请认证</el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="scope.row.carStatus == 3 && buttonLimits.includes(132)" @click="reserveCarButton(scope.$index, scope.row)">车辆预定
              </el-button>
              <el-button size="mini" type="success" plain icon="el-icon-sell" style="margin-top: 5px" v-if="scope.row.carStatus == 2 && buttonLimits.includes(133)" @click="putStorage(scope.$index, scope.row)">上传销售照片
              </el-button>
              <!-- <el-button
                size="mini"
                type="warning"
                ...
                @click="hostlingAuditButton(scope.$index, scope.row)"
              >整备审核</el-button>-->
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    applyAuthentication(index, row) {
      ...
      // console.log((nowTime - lastmaintenanceDate) / 1000 / 60 / 60 / 24 / 30 > 6);
      if (
        lastTime > nowTime ||
