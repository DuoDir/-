<template>
  <el-container class="customEdit" v-loading="loading">
    <el-header class="header">客户编辑</el-header>
    <el-main>
...
export default {
    ...
    methods: {
      ...
      echo() {
        ...
        this.loading = true
        // 客户信息
        this.$axios.post(this.pathItem.customerInfo,{id: this.form.id}).then((res) => {
          this.loading = false
          if (res.code == 0) {
            this.form.name = res.result.name==null?'':res.result.name // 客户名称
            this.form.phone = res.result.phone==null?'':res.result.phone // 客户电话
            this.form.idcard = res.result.idcard==null?'':res.result.idcard // 身份证号
            this.form.birthday = res.result.birthday==null?'':res.result.birthday // 客户生日
            this.form.wx = res.result.wx==null?'':res.result.wx // 微信号
            this.form.sex = res.result.sex==null?'':res.result.sex // 客户性别
            this.form.classify = res.result.classify==null?'1':res.result.classify // 客户分类
            this.form.callTime = res.result.callTime==null?'':res.result.callTime // 接待时间
            this.form.nature = res.result.nature==null?'':res.result.nature // 客户性质
            this.form.businessName = res.result.businessName==null?'':res.result.businessName // 企业名称
            this.form.level = res.result.level==null?'':res.result.level // 客户等级
            this.form.source = res.result.source==null?'':res.result.source // 客户来源 选择框
            if (res.result.source == '13') { this.form.source = '' } // 13  说明是 其他来源
            this.form.sourceBak = res.result.sourceBak==null?'':res.result.sourceBak // 客户来源 文本框
            this.form.sponsor = res.result.sponsor==null?'':res.result.sponsor // 介绍人
            this.form.salesAdvisorId = res.result.salesAdvisorId==null?'':res.result.salesAdvisorId // 销售顾问
            this.form.vehicleModel = res.result.vehicleModel==null?'':res.result.vehicleModel // 已看车型
            this.form.provinceCode = res.result.provinceCode==null?'':res.result.provinceCode // 省id
            this.form.province = res.result.province==null?'':res.result.province // 省
            this.form.detail = res.result.detail==null?'':res.result.detail // 详细地址
            this.form.bak = res.result.bak==null?'':res.result.bak // 备注信息
            if (res.result.cityCode) {
              this.getCity(res.result.provinceCode).then(eve => {
                if (eve) {
                  this.form.cityCode = res.result.cityCode // 市id
                  this.form.city = res.result.city // 市
                }
              })
            } else {
              this.form.cityCode = ''
        ...
        // 客户需求
        this.$axios.post(this.pathItem.customerNeedInfo,{customerId: this.form.id}).then((res) => {
          if (res.code == 0) {
            if (!res.result) {
              return;
            }
            this.form.needid = res.result.id==null?'':res.result.id // 需求id
            this.form.needType = res.result.needType==null?'':res.result.needType // 需求类型
            this.form.transmission = res.result.transmission==null?'':res.result.transmission // 变速器要求
            this.form.airDisplacementLow = res.result.airDisplacementLow==null?'':res.result.airDisplacementLow // 排气量 低
            this.form.airDisplacementUp = res.result.airDisplacementUp==null?'':res.result.airDisplacementUp // // 排气量 高
            this.form.airDisplacementUnit = res.result.airDisplacementUnit==null?'':res.result.airDisplacementUnit // 排气量 单位
            
            this.form.colorBak = res.result.colorBak==null?'':res.result.colorBak // 其他颜色
            this.form.ageLow = res.result.ageLow==null?'':res.result.ageLow // 车龄要求 低
            this.form.ageUp = res.result.ageUp==null?'':res.result.ageUp // 车龄要求 高
            this.form.mileageLow = res.result.mileageLow==null?'':res.result.mileageLow // 里程 低
            this.form.mileageUp = res.result.mileageUp==null?'':res.result.mileageUp // 里程 高
            this.form.priceLow = res.result.priceLow==null?'':res.result.priceLow // 价格 低
            this.form.priceUp = res.result.priceUp==null?'':res.result.priceUp // 价格 高
            this.form.isStage = res.result.isStage==null?'':res.result.isStage // 是否分期
            this.form.downPaymentRatio = res.result.downPaymentRatio==null?'':res.result.downPaymentRatio // 首付比例
            
            this.form.color = res.result.color==null?'':res.result.color // 颜色要求
            if (res.result.color != '' && res.result.color != null) { // 颜色回显
              let clsList = res.result.color.split(',') // 转换为数组
              this.colorList.forEach( (items,indexs) => {
                clsList.forEach( (item,index) => {
                  if (items.label == item) {
                    this.colorList[indexs].flag = true
                  }
                })
              })
            }

            if (res.result.brandId != '' && res.result.brandId != null) {
              this.form.brandId = (res.result.brandId).split(',') // 意向品牌
            }
            if (res.result.seriesId != '' && res.result.seriesId != null) {
              this.getCarSetByCarId().then( eve => {
                if (eve) {
                  this.form.seriesId = (res.result.seriesId).split(',') // 意向车系
                  console.log(this.form.seriesId)

                  if (res.result.modelId != '' && res.result.modelId != null) { 
                    this.getCarModelByCarSetId().then( eve => {
                      if (eve) {
                        this.form.modelId = (res.result.modelId).split(',') // 意向车型
                        console.log(this.form.seriesId)
                      }
                    })
                  }
      ...
      Save() {
        ...
        this.$axios.post(this.pathItem.editCustomer,params).then((res) => {
          if (res.code == 0) {
      ...
      // 客户需求保存
      saveCustomerNeed() {
        if (this.form.needid == '') {  // 如果客户没有填需求  直接成功
          this.successMsg('保存成功')
          let customEdit = {
            name: "customEdit",
          }
          this.$store.dispatch("delVisitedViews", customEdit).then(() => {
            this.$router.push({
              path: '/customDetail',
        ...
        this.$axios.post(this.pathItem.editCustomerNeed,data).then((res) => {
          this.loading = false
          if (res.code == 0) {
            this.successMsg('保存成功')
            let customEdit = {
      ...
      async getCity(id) {
        ...
        await this.$axios.post(this.pathItem.querySystemCityList,{parentId:id}).then((res) => {
          if (res.code == 0) {
            let data = res.result.systemCityList
      ...
      async getCarSetByCarId() {
        ...
        this.carSetList = []
        await this.$axios.post(this.pathItem.certiAllCarModel,{makeId: this.form.brandId[0]}).then((res) => {
          if (res.code == 0) {
            res.result.forEach(item => {
      ...
      async getCarModelByCarSetId() {
        ...
        await this.$axios.post(this.pathItem.certiCarModelList,{modelId: this.form.seriesId[0]}).then((res) => {
          if (res.code == 0) {
            res.result.forEach(item => {
      ...
      querySystemUserInfoDealersListOneTable() {
        this.$axios.post(this.pathItem.querySystemUserInfoDealersListOneTable,{roleTypes: 5}).then((res) => {
          if (res.code == 0) {
